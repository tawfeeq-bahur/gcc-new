...
export function Sidebar() {
  ...
  return (
    <>
      {/* Mobile Toggle */}
      <button
        className="fixed top-4 left-4 z-50 p-2 rounded-lg bg-slate-900 border border-slate-800 md:hidden"
      ...
      <aside
        className={cn(
          "fixed md:relative z-40 h-screen bg-slate-900/95 backdrop-blur-sm border-r border-slate-800 transition-all duration-300",
      ...
      >
        ...
        <div className="p-6 border-b border-slate-800">
          <Link href="/dashboard" className="flex items-center gap-3">
            ...
            {!isCollapsed && (
              <div>
                <h1 className="text-lg font-bold text-white">GCC-Ready</h1>
                <p className="text-xs text-slate-500">Talent Discovery</p>
              </div>
            )}
          </Link>
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          {!isCollapsed && (
            <div className="px-4 py-3 rounded-xl bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-indigo-500/20">
              <p className="text-xs text-slate-400">GCC Hackathon 2025</p>
